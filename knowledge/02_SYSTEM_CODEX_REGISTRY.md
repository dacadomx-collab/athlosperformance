...
> **Última actualización:** 2026-08-12 — Fase 29: Unificación definitiva de navegación (header único, tabs del Dashboard ocultos), fix de HTML inválido en modal de Testimonios, y 3 bugs WCAG reales adicionales (`--ssos-estado-verde/rojo/ambar`)
...
| `08_schema_testimonios.sql` | `testimonios_clientes` | — | Módulo Casos de Éxito (2026-08-12): reseñas de clientes con foto para el carrusel público de la Landing Page. Ver detalle completo en "Fase 26" más abajo. |
...
### Fase 26 — Módulo Casos de Éxito / Testimonios (2026-08-12)

> Ejecutado bajo Excepción al Mandamiento 9 (autorización humana explícita para extender el schema).
> Backend CRUD + API pública + carrusel Next.js, construido siguiendo 1:1 los patrones ya establecidos
> (`configuracion.php` para el CRUD con modales Bootstrap, `athlos_score.php`/`agenda_publica.php` para
> el contrato de API pública, `ssos-client.ts` para el cliente fetch del sitio estático).

| Archivo | Ruta | Estado | Descripción |
| :--- | :--- | :--- | :--- |
| `08_schema_testimonios.sql` | `knowledge/sql/` | Producción (local) | Tabla `testimonios_clientes` (ver columnas abajo) + seed de 3 testimonios mock. Aplicada y verificada contra la copia local de `tourfindycom_athlosp_db` (XAMPP); pendiente de aplicación manual en cPanel por el Arquitecto/Super Admin, mismo protocolo que `07_schema_configuracion_agenda_publica.sql` (Fase 25). |
| `testimonios/index.php` | `public/ssos/testimonios/index.php` | Producción (local) | CRUD completo (`require_role('admin','super_admin','coach')`): alta/edición vía modales Bootstrap (patrón de `dashboard/index.php`), "Eliminar" implementado como **soft delete** (`estatus → 'inactivo'`, nunca `DELETE` físico) con acción inversa "Reactivar" — mismo criterio de auditoría que `catalogo_servicios`/`membresias`. |
| `guardar_foto_testimonio()` / `eliminar_foto_testimonio()` | `public/ssos/testimonios/index.php` | Producción (local) | Subida segura: MIME real certificado con `finfo_file()` sobre el contenido (nunca la extensión ni el `Content-Type` del navegador, ambos falsificables), límite 2MB, nombre de archivo generado con `bin2hex(random_bytes(16))` (nunca el nombre original), guardado en `public/media/testimonios/`. Al editar, la foto vieja sólo se borra tras confirmar el `UPDATE` y sólo si se subió una nueva. Verificado en local: subida de PNG válido (éxito, archivo con nombre hasheado en disco), subida de texto plano disfrazado con extensión `.jpg` (rechazado, `"Formato de imagen no permitido"`, cero archivos escritos). |
| `public/media/testimonios/.htaccess` | `public/media/testimonios/` | Producción (local) | Defensa en profundidad: `Require all denied` sobre extensiones ejecutables (`.php`, `.cgi`, `.py`, etc.) y `php_flag engine off`, aunque la validación de MIME ya impide que llegue un archivo no-imagen a este directorio. |
| `ssos_asset_media()` | `public/ssos/config/helpers.php` | Producción (local) | Nuevo helper: URL absoluta hacia `public/media/*` desde dentro de `public/ssos/*` (sube 1 nivel desde `ssos_base_url()`, que siempre termina en `.../ssos`) — distinto de `ssos_asset_repo()` (sube 2 niveles, para `assets/` en la raíz del repo) porque `media/` es HERMANO de `ssos/` dentro de `public/`, no un directorio externo. Misma independencia de entorno (local XAMPP / producción cPanel) que el resto de la familia `ssos_asset*()`. |
| `testimonios_publicos.php` | `public/ssos/api/testimonios_publicos.php` | Producción (local) | `GET` — sin autenticación (lectura pública por diseño, mismo criterio que `agenda_publica.php`). Devuelve sólo `estatus = 'activo'`, ordenado por `orden_visualizacion ASC, fecha_testimonio DESC`. Contrato JSON (snake_case, `JSON_UNESCAPED_UNICODE` ya activo en `api_respond()`): `{ testimonios: [{ id_testimonio, nombre_cliente, comentario, foto_url, fecha_testimonio }] }`. `foto_url` es `null` si el testimonio no tiene foto. |
| Menú lateral (`🌟 Casos de Éxito`) | `public/ssos/partials/header.php` | Producción (local) | Entrada agregada al bloque de nav visible para `coach`/`admin`/`super_admin` (mismo bloque que Agenda/Sesiones del Día), enlazando a `testimonios/index.php`. |
| `.ssos-testimonio-thumb` / `.ssos-testimonio-resena` | `public/ssos/css/main.css` | Producción (local) | Miniatura circular 3rem con fallback de iniciales (mismo patrón que avatares de coach), y truncado de una línea (`text-overflow: ellipsis`) para la columna de reseña en la tabla del listado. |
| `fetchTestimoniosPublicos()` / `TestimonioPublico` | `lib/ssos-client.ts` | Producción (local) | Cliente fetch sin API key (mismo criterio que `submitLead()` — sitio 100% estático), usa el mismo `SSOS_API_BASE` ya declarado en el archivo. Mapea snake_case (wire) → camelCase (`idTestimonio`, `nombreCliente`, `fotoUrl`, `fechaTestimonio`). Falla en silencio (`[]`) ante cualquier error de red/formato — el carrusel es contenido de refuerzo, nunca debe romper la Landing Page si el BackOffice está caído. |
| `TestimonialCarousel` | `components/athlos/TestimonialCarousel.tsx` | Producción (local) | `"use client"`, sin props. Sección `#testimonios`, insertada en `app/page.tsx` entre `EvidenceSection` (`#autoridad`) y `FinalCtaSection`. `fetch` en `useEffect` al montar; devuelve `null` (no renderiza nada) mientras carga o si no hay testimonios activos — nunca bloquea ni rompe el resto de la página. Auto-avance cada 12s (rango 10-15s solicitado) vía `setInterval`, pausado en `prefers-reduced-motion`, en hover (`onMouseEnter`/`onMouseLeave`) y en foco de teclado (`onFocus`/`onBlur`) sobre el carrusel. Controles manuales: flechas prev/next + dots `role="tablist"`. Avatar con `next/image` (`unoptimized`, ya global en `next.config.mjs`) o iniciales si `fotoUrl` es `null`. |
| `.testimonial-carousel*` / `.testimonial-card*` | `styles/athlos-theme.css` | Producción (local) | Card estilo `.timeline__phase` (glass + `fade-up` en cada cambio de testimonio vía `key` de React), avatar circular, controles (flechas + dots) con el mismo lenguaje visual que `.segment-tab`/`.back-to-top`. 100% variables de tema (`--surface`, `--athlos-line`, `--athlos-turquoise*`), sin anchos fijos en px, `max-width` en `rem`. |

**Verificado localmente (no en navegador real — ver Informe Forense en la entrega):** ciclo CRUD completo vía HTTP (`curl`) contra el BackOffice autenticado — alta con foto, edición preservando la foto si no se sube una nueva, rechazo de archivo no-imagen disfrazado, soft delete (desaparece de `testimonios_publicos.php`) y reactivación. `pnpm build` (producción, con chequeo de tipos) verde. Servidor `next dev` existente recompiló `TestimonialCarousel`/`app/page.tsx` sin errores de compilación. **Pendiente de verificación visual en navegador real** (auto-avance, pausa en hover, layout responsivo, temas claro/oscuro) — no se realizó por no contar con herramienta de navegador en este entorno de ejecución.

### Fase 27 — Auditoría de Navegación SSOS y Módulo Equipo del Laboratorio (2026-08-12)

> Bajo Mandamiento 19 (BD remota unificada), esta fase se auditó y construyó contra el schema real
> vía lectura de código + `php -l` + verificación cruzada de IDs HTML↔JS. **No se realizó prueba
> funcional vía HTTP con sesión real** (sin credenciales de producción disponibles y sin entorno
> local autorizado para simularla) — ver Informe Forense en la entrega para el detalle de qué
> quedó verificado estáticamente vs. pendiente de prueba en navegador real por el Arquitecto.

| Archivo | Ruta | Estado | Descripción |
| :--- | :--- | :--- | :--- |
| `.ssos-back-to-top` (fix de z-index) | `public/ssos/css/main.css` | Producción (local) | **Bug real encontrado y corregido:** `z-index: 1050` (por encima del `.offcanvas` de Bootstrap, 1045) hacía que el FAB "volver arriba" (fixed, bottom/right:20px) interceptara clics destinados a los últimos ítems del menú hamburguesa (ej. "Cerrar sesión", mismo lado `offcanvas-end`) cuando el usuario había scrolleado >300px antes de abrir el menú. Bajado a `1030` (capa "fixed" de Bootstrap, por debajo de offcanvas-backdrop 1040 y offcanvas 1045). |
| `$verEquipo`, tab "🧑‍💼 Equipo del Laboratorio" | `public/ssos/dashboard/index.php` | Producción (local) | Nueva pestaña compartida (`admin` + `super_admin`) que **consolida** dos flujos de gestión de staff que antes vivían duplicados a medias: la tabla de sólo-lectura + alta (sólo `super_admin`, en "Dirección y Control") y el botón de alta de sólo-Coach (sólo `admin`, en "Clientes y Membresías"). Ambos removidos de sus tabs originales; "Dirección y Control" conserva únicamente sus widgets de BD y la bitácora de sesiones (contenido exclusivo de Dirección). |
| `editar_miembro_equipo`, `resetear_password_equipo`, `cambiar_estatus_equipo` (handlers) | `public/ssos/dashboard/index.php` | Producción (local) | 3 acciones POST nuevas con candado de seguridad reforzado en el SERVIDOR (nunca sólo en la UI): ningún actor puede tocar una cuenta `super_admin` (verificado consultando `obtenerClaveRolUsuario()` ANTES de cualquier `UPDATE`), el selector de rol sólo ofrece `coach`/`admin` (super_admin nunca es una opción asignable, ni siquiera para `super_admin` como actor), y las 3 acciones bloquean explícitamente el auto-objetivo (`id_usuario === $_SESSION['id_usuario']`) para evitar auto-bloqueo/auto-degradación accidental. `resetear_password_equipo` reutiliza `requiere_cambio_password` (ya existente en el schema desde `01_schema_usuarios_rbac.sql`) para forzar cambio en el siguiente login. `cambiar_estatus_equipo` es soft delete puro (`usuarios.activo`), nunca `DELETE` físico — mismo criterio que `testimonios_clientes`/`catalogo_servicios`. |
| `$rolesCreablesPorRol` (ampliado) | `public/ssos/dashboard/index.php` | Producción (local) | `admin` ahora puede dar de alta cuentas `admin` además de `coach` (antes sólo `coach`) — decisión explícita del punto 3 de la orden ("Admin o Coach"). Sigue sin poder crear `super_admin` ni `atleta`. |
| `$miembrosEquipo` (query con candado en el SELECT) | `public/ssos/dashboard/index.php` | Producción (local) | Cuando el actor es `admin`, la consulta agrega `AND r.clave_rol != 'super_admin'` — el candado de super_admin aplica también a nivel de datos: `admin` nunca recibe del servidor ni las filas de cuentas `super_admin` (no sólo las tiene ocultas en el HTML). |
| Filtro "Ocultar suspendidos/inactivos" | `public/ssos/dashboard/index.php`, `public/ssos/js/main.js` | Producción (local) | Checkbox `data-ssos-ocultar-suspendidos` + `data-estatus` en cada `<tr>` de la tabla de Clientes; JS 100% client-side (`fila.style.display`), sin recargar la página — mismo patrón ya usado en el sidebar de coaches de la Agenda. |
| `modalEquipoEditar` / `modalEquipoResetPassword` (modales compartidos) | `public/ssos/dashboard/index.php`, `public/ssos/js/main.js` | Producción (local) | Un solo modal por acción, rellenado vía evento `show.bs.modal` leyendo los `data-*` del botón que lo abrió — mismo patrón ya probado en `modalEditarAtleta` (Fase 6), evita generar un modal por fila. |
| Navegación en Agenda (punto 1 de la orden) | `public/ssos/agenda/index.php` | Sin cambios — verificado | **Premisa de la orden no confirmada en el código:** el archivo ya incluye `partials/header.php` (línea 35 antes de esta fase) — el menú hamburguesa con navegación global (incluida "🌟 Casos de Éxito") ya está presente y funcional en la vista de Agenda. La única ocultación deliberada en esa vista es el breadcrumb secundario "Volver al Dashboard" en móvil (`.ssos-breadcrumb--full-viewport-movil`, Fase 24) — decisión de espacio documentada en el propio CSS, no un bloqueo de navegación. |
| Menú "🌟 Casos de Éxito" para `admin` (punto 4 de la orden) | `public/ssos/partials/header.php` | Sin cambios — verificado | Ya construido correctamente en la Fase 26: visible para `coach`/`admin`/`super_admin` (mismo bloque condicional que el enlace de Agenda). |

**Pendiente de verificación en navegador real por el Arquitecto/usuario (no realizada en esta fase, ver limitación arriba):** clics reales en el menú hamburguesa tras el fix de z-index, flujo completo de alta/edición/reset de contraseña/soft-delete de un miembro del equipo con una sesión real de `admin` y de `super_admin`, y el filtro de suspendidos en la tabla de Clientes.

### Fase 28 — Retención de Pestaña, Nav de Escritorio y Auditoría Responsiva/WCAG (2026-08-12)

> Misma limitación de verificación que la Fase 27: sin credenciales de producción disponibles,
> todo lo de esta fase se verificó por lectura de código + `php -l`/`node --check` + contraste
> WCAG calculado matemáticamente (fórmula de luminancia relativa oficial), no en navegador real.

| Archivo | Ruta | Estado | Descripción |
| :--- | :--- | :--- | :--- |
| `$tabActivaPorDefecto` (retención de pestaña) | `public/ssos/dashboard/index.php` | Producción (local) | **Bug real confirmado:** antes SIEMPRE volvía a `$tabsDisponibles[0]` tras cualquier POST (crear/editar equipo, atleta, etc.), sin importar en qué pestaña trabajaba el usuario. Ahora lee `$_POST['tab_origen'] ?? $_GET['tab']`, validado contra `array_column($tabsDisponibles, 'id')` (nunca se confía en el valor recibido sin verificar que sea una pestaña real disponible para ese rol) antes de usarlo como pestaña activa del render. |
| `initRetencionPestanaActiva` (submit listener genérico) | `public/ssos/js/main.js` | Producción (local) | Inyecta/actualiza un campo oculto `tab_origen` en CUALQUIER `<form method="post">` de la página, leyendo `.tab-pane.active[id^='pane-']` justo antes del envío — cero campos ocultos manuales por formulario, cero riesgo de que un formulario nuevo "se le olvide" el mecanismo. No-op inofensivo en páginas sin sistema de pestañas (`testimonios/index.php`, `login.php`, etc. — el selector no encuentra nada). |
| Sync de hash en `shown.bs.tab` | `public/ssos/js/main.js` | Producción (local) | Bootstrap NO actualiza `location.hash` al cambiar de tab (sólo alterna clases CSS) — se agregó `history.replaceState()` en cada `shown.bs.tab` para que el hash refleje siempre la pestaña realmente visible (recarga/F5/compartir enlace ya no pierden el contexto). |
| `$ssos_nav_items` (refactor DRY) + `.ssos-navbar-desktop-nav` | `public/ssos/partials/header.php`, `public/ssos/css/main.css` | Producción (local) | **Punto 2a:** nueva barra de navegación horizontal visible desde 992px (breakpoint `lg`, ya usado en el resto de `main.css`) con los mismos ítems que el Offcanvas — incluye "🌟 Casos de Éxito" a la vista permanente en escritorio. El Offcanvas/hamburguesa sigue existiendo en todos los anchos (da acceso a rol/nombre/cerrar sesión); la barra nueva es un complemento, no un reemplazo. Ambas superficies ahora se generan del MISMO arreglo `$ssos_nav_items` (antes cada una tenía su propio HTML hardcodeado — la causa estructural de que "Casos de Éxito" pudiera faltar en una de las dos). |
| Re-auditoría de clics (punto 2b) | — | Sin cambios — verificado | El fix de `.ssos-back-to-top` (z-index 1050→1030, Fase 27) sigue intacto. Segunda pasada no encontró otros conflictos: los únicos `event.preventDefault()` de `main.js` están guardados detrás de `if (!celda) return` (sólo aplican al drag-and-drop de la Agenda, nunca a enlaces de navegación), y los únicos `pointer-events: none` restantes son de patrones ya correctos (fade de FAB, checkbox nativo oculto del checklist Pie de Cancha). |
| `.ssos-table-card .btn-sm` (área táctil) | `public/ssos/css/main.css` | Producción (local) | **Punto 3.2:** el `.btn-sm` de Bootstrap mide ~31px de alto (por debajo del mínimo táctil de 44px) — confirmado por cálculo contra el CSS real de Bootstrap 5.3.3, no una suposición. Ampliado a 44×44px mínimo, sólo en viewports ≤767.98px (en escritorio la densidad compacta es la UX correcta para tablas de datos con mouse) y sólo vía `padding`/`min-height` (el tamaño visual del texto/ícono no cambia). |
| Overflow de tablas, ARF-Grid (puntos 3.1 y 3.3) | — | Sin cambios — verificado | Las 16 `<table>` del BackOffice SSOS ya están 100% envueltas en `.ssos-table-card` (`overflow-x:auto`, incluye la matriz de Agenda vía `.ssos-agenda-matriz-wrap`, que además ya tenía `min-width:0` — el detalle que evita que una tabla ancha reviente su contenedor flex) o en `.table-responsive` de Bootstrap. `.ssos-widget-grid`/`.pdc-grid` ya implementan el patrón ARF-Grid exacto solicitado (`flex; flex-wrap:wrap; justify-content:center` + hijos con `flex-basis` fluido, cero anchos fijos en px). |
| `--athlos-turquesa-text` (fix WCAG real) | `public/ssos/css/main.css` | Producción (local) | **Punto 3.4 — bug real encontrado por cálculo matemático de contraste** (fórmula de luminancia relativa WCAG): `--athlos-turquesa` (`#00B8C9`) como color de TEXTO sobre fondos claros mide **2.42:1** — no pasa el mínimo de 4.5:1. De los 8 usos de `color: var(--athlos-turquesa)` en el archivo, 6 ya estaban correctamente aislados bajo `[data-theme="dark"]` (ahí sí pasa, 6.16:1) y no se tocaron; los 2 restantes (`.ssos-offcanvas .nav-link.active`, `.ssos-tabs .nav-link:hover`) aplicaban sin aislar en modo claro — el default de la app (`:root { color-scheme: light }`, sin necesitar `[data-theme]`). Se introdujo `--athlos-turquesa-text` (`#00707D` en claro, ~5.5:1; reutiliza el turquesa original en oscuro) — mismo valor y mismo patrón ya validado en producción en el sitio público (`--athlos-turquoise-text`, Módulo 7 de `styles/athlos-theme.css`), sólo que con la polaridad invertida porque aquí el claro es el tema por defecto (en el sitio público lo es el oscuro). Colores usados como `border-color`/`accent-color` (no texto) no se tocaron — el umbral de contraste no-texto (3:1) es una categoría distinta, fuera del "mínimo 4.5:1" pedido explícitamente. |

**Diseño elegido para "retención de pestaña" — nota de transparencia:** la orden sugería `header('Location: ...?tab=X&status=success')` (patrón PRG). Se optó por **mantener el patrón ya existente** (procesar y renderizar en la misma petición, sin redirect) porque ya funciona así en los ~10 handlers existentes y migrar a PRG habría requerido introducir mensajes flash vía `$_SESSION` en todos ellos — más superficie de cambio y de regresión por el mismo resultado funcional. `?tab=`/`tab_origen` sigue siendo la fuente de verdad de qué pestaña mostrar, tal como se pidió; sólo cambia el mecanismo de transporte (mismo request vs. redirect).

### Fase 29 — Auditoría "Calidad Total": Unificación Definitiva de Navegación, Modal de Testimonios y WCAG 100/100 (2026-08-12)

> **Pivote de arquitectura respecto a la Fase 28, documentado explícitamente:** la Fase 28 auditó
> un reporte de "triple menú" y resolvió recortando `$ssos_nav_items` a sólo páginas separadas
> (Agenda/Testimonios), dejando la navegación interna del Dashboard (`#control`, `#clientes`, etc.)
> exclusivamente en la barra de pestañas propia de `dashboard/index.php`. Esta fase pide la
> dirección **opuesta**: una única barra de navegación en el header que absorba TODAS las
> secciones (incluidas las del Dashboard), con la barra de pestañas propia oculta por completo.
> No es una corrección de un error de la Fase 28 — es un cambio de decisión de producto explícito
> del Arquitecto, implementado sin fricción porque ambos diseños comparten la misma base técnica
> (`$ssos_nav_items` como fuente única + `activarTabDesdeHash()`).

| Archivo | Ruta | Estado | Descripción |
| :--- | :--- | :--- | :--- |
| `$ssos_nav_items` (restaurado a 7 ítems) | `public/ssos/partials/header.php` | Producción (local) | Vuelve a incluir las 5 secciones internas del Dashboard (`#control`, `#clientes`, `#equipo`, `#pie-de-cancha`, `#herramientas`) junto a Agenda y Casos de Éxito — ahora es la ÚNICA fuente de navegación, en la barra de escritorio y el Offcanvas. |
| `.ssos-tabs { display: none; }` | `public/ssos/css/main.css` | Producción (local) | La barra de pestañas nativa de `dashboard/index.php` se oculta visualmente en TODOS los anchos — pero se mantiene sin alterar en el DOM/HTML/JS: Bootstrap sigue necesitando esos botones (`data-bs-target="#pane-X"`) para que `activarTabDesdeHash()` (ya existente, sin cambios) pueda invocar `new bootstrap.Tab(boton).show()` cuando el usuario navega por hash desde el header, sin importar si ya estaba en el Dashboard o llegó de otra página. Cero JavaScript nuevo — se reutilizó 100% el mecanismo de hash ya construido en la Fase 6/28. |
| Re-auditoría de clics (main.js, línea por línea) | — | Sin cambios — verificado | Relectura completa y secuencial de las 526 líneas de `main.js`: todo acceso DOM potencialmente arriesgado (`querySelector(...).value = ...`) vive dentro de un callback de `addEventListener`, nunca en código síncrono de nivel superior — un error dentro de un handler no puede impedir que los `addEventListener` posteriores del archivo se registren. El fix de z-index (Fase 27) sigue intacto. Hipótesis registrada para el Arquitecto: el "triple menú" de la Fase 28 pudo ser la causa real de la sensación de "clics que no responden" — con 3 superficies de navegación redundantes visibles, un clic que sí funcionaba en una superficie podía parecer "no hacer nada" si otra superficie seguía mostrando el estado anterior. |
| Modal "Editar Testimonio" (HTML inválido corregido) | `public/ssos/testimonios/index.php`, `public/ssos/js/main.js` | Producción (local) | **Causa raíz real del "modal transparente" reportado, encontrada por lectura de código, no adivinada:** el modal de edición se insertaba como `<div class="modal">` hijo directo de `<tbody>` (uno por fila) — HTML inválido (un `<tbody>` sólo admite `<tr>`); el navegador lo "repara" reubicándolo en el DOM (foster parenting), con posicionamiento/stacking impredecible. Corregido convirtiéndolo en un único modal compartido fuera de la tabla, rellenado vía `data-*` — mismo patrón que `modalEquipoEditar`/`modalEditarAtleta`. `.modal-content` YA tenía fondo opaco correcto por tema desde la Fase 28 (verificado, no era el problema); se mantiene el refuerzo explícito de z-index como documentación de la garantía pedida. |
| Vista previa de nombre de archivo (`accept="image/*"` MIME-based) | `public/ssos/testimonios/index.php`, `public/ssos/js/main.js` | Producción (local) | Confirmado: `enctype="multipart/form-data"` ya presente en ambos formularios, sin `preventDefault()`/`stopPropagation()` interfiriendo con el input de archivo. Se agregó `data-ssos-preview-nombre-archivo` + listener genérico de `change` que muestra "📎 nombre-del-archivo.jpg" antes de guardar, en alta y edición. |
| `--ssos-estado-verde` / `--ssos-estado-rojo` / `--ssos-estado-ambar` | `public/ssos/css/main.css` | Producción (local) | **3 bugs WCAG reales adicionales, encontrados por el mismo cálculo de luminancia relativa de la Fase 28 (no los que la orden adivinó — `#000`/`#1f2937`/`#333` no existen en este código — pero de la misma clase real):** `#1E9E5A` (verde "activo") mide 3.29:1 en modo claro (falla); `#D64545` (rojo "suspendido") mide 4.18:1 en claro y 3.91:1 en oscuro (falla en AMBOS); `#D68910` (ámbar "alerta") mide 2.82:1 en claro (falla). Los 3 eran valores fijos usados como color de texto sobre fondos que sí cambian por tema. Reemplazados por variables `--ssos-estado-*` con un tono por modo (el rojo necesitó una luminancia opuesta en cada tema — no existe un solo rojo que pase contra fondo blanco Y fondo casi-negro). Aplicados en `.ssos-estatus-select--activo/--suspendido` (Clientes) y `.ssos-agenda-cliente-mes--alerta` (sidebar de Agenda). `ssos-auth.css` (login) y `reporte.css` (reporte público) se auditaron con el mismo método — sin hallazgos, todos sus colores fijos son texto sobre fondos igualmente fijos (botones institucionales, impresión), no sobre fondos que cambian por tema. `styles/athlos-theme.css` no se tocó: no lo carga ninguna página SSOS (confirmado por grep), es exclusivo del sitio Next.js y ya tiene su propia auditoría completa (Módulo 7). |

**Pendiente de verificación en navegador real (misma limitación de las Fases 27/28 — sin sesión de producción disponible):** que la barra unificada de 7 ítems no rompa el layout en 992-1200px (tiene `flex-wrap` como red de seguridad, sin verificar visualmente), que los clics del Offcanvas se sientan "infalibles" ahora que sólo hay una superficie de navegación, y que el modal de Testimonios ya no se vea transparente.
...
| `TestimonialCarousel` | `components/athlos/TestimonialCarousel.tsx` | UI / Landing (Client) | Producción | Ver Fase 26. Sin props. Sección `#testimonios` con carrusel auto-avanzado (12s) de reseñas de clientes, consumidas en runtime desde `testimonios_publicos.php` vía `fetchTestimoniosPublicos()`. |
| `fetchTestimoniosPublicos()` / `TestimonioPublico` | `lib/ssos-client.ts` | Util / Content (Client) | Producción | Ver Fase 26. Cliente fetch del endpoint público de testimonios, reutiliza `SSOS_API_BASE` ya declarado en el archivo para `submitLead()`. |
