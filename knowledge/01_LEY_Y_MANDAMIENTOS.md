...
19. **Prohibición de alterar conexiones de bases de datos funcionales o entornos remotos por entornos locales simulados, sin autorización explícita.** Autorizado el 2026-08-12. **Nota de reconciliación con el Mandamiento 13:** este mandamiento reemplaza, para este proyecto, el modelo de 3 entornos (Local con datos falsos / Staging / Producción) descrito en el Mandamiento 13 — el entorno local de desarrollo se conecta directamente a la base de datos remota real (`chir205.websitehostserver.net`) en vez de a una copia local sembrada. Cualquier base de datos local (usuarios MySQL locales, `.env` alternos, hosts `localhost`) requiere autorización explícita antes de crearse o usarse; no se asume por defecto.